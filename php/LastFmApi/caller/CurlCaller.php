...
final class CurlCaller extends Caller {
	...
	private function __construct(){
		$this->curl  = curl_init();
		$this->cache = new DiskCache('diskcache');
		...
		curl_setopt($this->curl, CURLOPT_PROXY, MUSEO_CONFIG_PROXY);
	}
	...
	protected function internalCall($params, $requestMethod = 'GET'){
		...
		if($this->cache != null &&
			...
			$responseRes = new SimpleXMLElement($response);
			...
			/* Cache it. */
			if($this->cache != null && $responseRes){
		...
		/* Assign SimpleXMLElement to response. */
